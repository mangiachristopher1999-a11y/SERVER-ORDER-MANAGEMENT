  ...
  reader.onload = function(evt) {
    try {
      ...
      if (imp && typeof imp === 'object') {
        CAT_KEYS.forEach(k => { if (Array.isArray(imp[k])) state.menu[k] = imp[k]; });
        if (save()) { renderMenu(); toast('Menu caricato con successo!'); }
      } else { throw new Error(); }
    } catch(err) { toast('Formato file non valido!'); }
  };
