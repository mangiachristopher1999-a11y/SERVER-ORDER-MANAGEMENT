...
// ⚠️ INCOLLA QUI IL TUO CONFIG PRESO DALLA CONSOLE DI FIREBASE
const firebaseConfig = {
  apiKey: "AIzaSyAVkrlEuxLqCK2VR7DRNyDt18XuNEGz2LQ",
  authDomain: "server-order-management.firebaseapp.com",
  databaseURL: "https://server-order-management-default-rtdb.europe-west1.firebasedatabase.app",
  projectId: "server-order-management",
  storageBucket: "server-order-management.firebasestorage.app",
  messagingSenderId: "282452538559",
  appId: "1:282452538559:web:a540db0c0278ba1d3c020a"
};
...
const dbRef = db.ref('ristorante/stato_app'); // Unico nodo per tutto lo stato

async function save() {
  try {
    // 💡 IL TRUCCO: JSON.stringify elimina automaticamente tutti i valori "undefined" 
    // che farebbero arrabbiare Firebase, creando un pacchetto dati perfetto.
    const cleanState = JSON.parse(JSON.stringify(state));
    
    await dbRef.set(cleanState);
    return true;
  } catch(e) {
...
// Caricamento in REAL-TIME: resta in ascolto per ogni modifica sul cloud
function load() {
  return new Promise((resolve) => {
    ...
    dbRef.on('value', (snapshot) => {
      ...
      if (parsed) {
        // 1. MENU: Forza la conversione in Array (Antidoto al bug di Firebase)
        if (parsed.menu) {
          CAT_KEYS.forEach(k => {
            if (parsed.menu[k]) {
              // Prende i dati e li ritrasforma in una lista pura, ignorando i "buchi"
              state.menu[k] = Object.values(parsed.menu[k]).filter(x => x !== null && x !== undefined);
            } else {
              state.menu[k] = [];
            }
          });
        ...
        if (parsed.currentShift !== undefined) state.currentShift = parsed.currentShift;
        
        // 2. ORDINI E PRENOTAZIONI: Forza la conversione in Array
        if (parsed.orders) {
          state.orders = Object.values(parsed.orders).filter(x => x !== null);
        } else {
          state.orders = [];
        }
        
        if (parsed.reservations) {
          state.reservations = Object.values(parsed.reservations).filter(x => x !== null);
        } else {
          state.reservations = [];
        }
      ...
      if (isFirstLoad) {
        ...
        if (currentTab !== 'stamp' && currentTab !== 'menu' && currentTab !== 'prenotazioni') {
          render(); 
        } else {
          renderPill();
        }
...
function renderOrders() {
  ...
  allSids.forEach(sid => {
    ...
    shiftMap[sid].forEach(o => {
      ...
      // Nuova logica: Order [1] sempre presente, dettagli opzionali
      const n = o.orderNumber || '?';
      let infoExtra = [];
      if (o.table) infoExtra.push(`Tav. ${esc(o.table)}`);
      if (o.customerName) infoExtra.push(esc(o.customerName));

      const titleDisplay = infoExtra.length > 0 
        ? `Order [${n}] • ${infoExtra.join(' — ')}` 
        : `Order [${n}]`;
      ...
      if (isOpen) {
        ...
        CAT_KEYS.forEach(key => {
          ...
          // Mappiamo gli indici originali PRIMA di filtrare, fondamentale per Firebase
          const itemsWithIndex = (cat.items || []).map((item, idx) => ({ ...item, originalIdx: idx })).filter(i => i.qty > 0);
          
          if (itemsWithIndex.length === 0 && !cat.notes) return;
          html += `<div class="oc-cat-lbl">${CATS[key]}</div>`;
          
          itemsWithIndex.forEach(i => {
            const statusClass = (i.status && i.status !== 'null') ? ` status-${i.status}` : '';
            
            // Abilita il click SOLO per le bevande
            let clickAttr = "";
            if (key === 'bevande') {
              const currentStatus = i.status || 'null';
              clickAttr = ` onclick="event.stopPropagation(); toggleDrinkStatus('${o.id}', '${key}', ${i.originalIdx}, '${currentStatus}')" style="cursor:pointer;"`;
            }
            
            html += `<div class="oc-line${statusClass}"${clickAttr}><span>${esc(i.name)}</span><span>${i.qty}× €${(i.price * i.qty).toFixed(2)}</span></div>`;
...
function exportOrder(id) {
  ...
  if (!o) return;
  const fileName = (o.table ? 'tavolo_'+o.table : 'comanda_'+(o.orderNumber||'')) + '_' + fmtTime(o.time).replace(':', '-') + '.txt';
...
function renderStamp() {
  ...
  let footerBtns = `<button id="btn-save-stamp" class="btn ${hasShift ? 'btn-black' : 'btn-muted'}" onclick="${hasShift ? 'saveStamp()' : 'toast(\'Attendi che l\\\'Admin avvii il turno\')'}" style="min-width:130px;">${hasShift ? saveBtnText : 'In attesa di Turno'}</button>`;
...
function saveStamp() {
  ...
  if (editingOrderId) {
    ...
    // Calcolo chirurgico del progressivo comanda per il turno corrente
    const shiftOrders = state.orders.filter(o => o.shiftId === shift.id);
    const nextNum = shiftOrders.length > 0 ? Math.max(...shiftOrders.map(o => o.orderNumber || 0)) + 1 : 1;

    const order = { id: uid(), shiftId: shift.id, orderNumber: nextNum, table, covers, customerName, time: new Date().toISOString(), categories, copertoPrice: state.copertoPrice || 0, total: calcTotal() };
...
function orderToMarkdown(o) {
  ...
  // Intestazione
  t += SEP2 + '\n';
  t += o.orderNumber ? ` COM. #${o.orderNumber}` : ` COMANDA`;
  if (o.table) t += `  ·  TAV. ${o.table}`;
...
function orderToText(o) {
  ...
  let t = '╔' + '═'.repeat(34) + '╗\n';
  const headerNum = o.orderNumber ? `COM. #${o.orderNumber}` : `COMANDA`;
  t += '║  ' + headerNum.padEnd(32) + '║\n';
  if (o.table) t += '║  Tavolo: ' + String(o.table).padEnd(24) + '║\n';
  if (o.customerName) t += '║  Nome: ' + String(o.customerName).substring(0, 25).padEnd(27) + '║\n';
...
function addReservation() {
  ...
  // Salvataggio pulito senza flag di notifica
  state.reservations.push({ id: uid(), name, table, covers, time });
...
// --- GESTIONE RAPIDA BEVANDE (Solo Cameriere: null <-> ready) ---
function toggleDrinkStatus(orderId, categoryKey, itemIndex, currentStatus) {
  vibra();
  // Ciclo binario: salta il giallo (prep). Se è pronto resetta, altrimenti imposta pronto (verde).
  const newStatus = (currentStatus === 'ready') ? null : 'ready';
  
  // Aggiornamento chirurgico su Firebase usando l'indice originale
  dbRef.child('orders').once('value', snapshot => {
    snapshot.forEach(childSnap => {
      const o = childSnap.val();
      if (o && o.id === orderId) {
        childSnap.ref.child(`categories/${categoryKey}/items/${itemIndex}/status`)
          .set(newStatus)
          .catch(err => console.error("Errore aggiornamento bevanda:", err));
      }
    });
  });
}
